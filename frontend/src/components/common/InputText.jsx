  ...
  return (
   ...
   {label && (<label>{label}</label>)}
      <input type={type} value={value} autoComplete={autoComplete}  name={name} onChange={handleOnChange} placeholder={placeholder} disabled={disabled}  
         className={`w-full px-4  py-3 bg-white border border-gray-300  rounded-lg focus:outline-blue-300   focus:ring-2 focus:ring-blue-100 ${hasError ? 'ring-red-100 outline-red-300 ring-3 border-red-300':''}`}
