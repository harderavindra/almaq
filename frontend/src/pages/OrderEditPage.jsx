...
import { FiCalendar, FiCheck, FiClipboard, FiClock, FiMapPin, FiPhone, FiPlus, FiTrash, FiTrash2, FiTruck, FiUser, FiX } from 'react-icons/fi';
          ...
          <table className="w-full border-collapse mt-10 rounded-lg overflow-hidden">
            ...
            <tbody>
              {items.map((itemGroup, idx) => {
                const { allDelivered, hasInvoice, invoiceId, farmer, items: subItems } = itemGroup;

                return (
                  <React.Fragment key={idx}>
                    {subItems.map((item, itemIdx) => (
                      <tr key={item._id} className="border-b border-gray-200 hover:bg-gray-50">
                        {/* Render Farmer Cell Only Once Per Group */}
                        {itemIdx === 0 && (
                          <td rowSpan={subItems.length} className="px-4 py-3 align-top border-r border-gray-200" align="top">
                            <div className="text-gray-700 flex items-center gap-1 capitalize">
                              <span className="text-blue-500 font-semibold">
                                {(currentPage - 1) * ITEMS_PER_PAGE + idx + 1}.
                              </span>
                              <strong>{farmer?.firstName} {farmer?.lastName}</strong><br />
                              <FiMapPin className="inline" /> {farmer?.taluka}
                              {farmer?.contactNumber ? (
                                <>
                                  <br /><FiPhone className="inline" /> {farmer.contactNumber}
                                </>
                              ) : (
                                ''
                              )}
                        ...
                        <td className="px-4 py-1">
                          <div className="flex items-center gap-2 text-sm text-gray-500">
                            <StatusBubble
                              status={item.status === 'Cancelled' ? 'error' : item.status === 'Delivered' ? 'success' : 'info'}
                        ...
                        {/* Delete Action */}
                        <td className="px-4 py-1 border-r border-l border-gray-200 text-center">
                        ...
                        {/* Invoice Button - only once per group */}
                        {itemIdx === 0 && allDelivered && (
                          <td rowSpan={subItems.length} className="px-4    py-3 text-right" valign="middle">
                            {hasInvoice && invoiceId ? (
                              ...
                              <Link
                                to={`/create-invoice/${order._id}/${farmer?._id}`}
